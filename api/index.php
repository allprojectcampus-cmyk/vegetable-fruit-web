...
// Silakan ganti URL di dalam tanda kutip dengan link merchant asli toko kamu
$linkGoFood     = "https://gofood.co.id/"; 
$linkGrabFood   = "https://food.grab.com/id/";
$linkShopeeFood = "https://shopeefood.co.id/";


// Fungsi untuk mengambil dan memformat data JSON dari Google Sheets
function fetchGoogleSheetData($url) {
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, $url);
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
    // Menonaktifkan verifikasi SSL untuk kelancaran di localhost (XAMPP)
    curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false); 
    $result = curl_exec($ch);
    curl_close($ch);

    // Google Sheets JSON biasanya dibungkus dalam fungsi javascript, kita perlu memotongnya
    $start = strpos($result, '{');
    $end = strrpos($result, '}');
...
if (isset($dataMenuRaw['table']['rows'])) {
    foreach ($dataMenuRaw['table']['rows'] as $row) {
        ...
        if ($namaMenu) { // Hanya ambil baris yang ada namanya
...
<head>
    ...
    <style>
        ...
        .hero-section {
            ...
            border-radius: 16px;
            padding: 60px 15px; /* Padding lebih kecil untuk HP */
            margin-top: 20px;
            text-align: center;
        }
        .hero-title {
        ...
        .product-img {
            height: 120px; /* Gambar lebih pendek di HP agar muat 2 kolom */
            object-fit: cover;
            width: 100%;
        ...
        .price-text {
            color: var(--primary);
            font-weight: 700;
            font-size: 0.9rem; /* Harga disesuaikan di HP */
        }
        .badge-kategori {
            background-color: var(--tertiary);
            color: #2b3234;
            font-weight: 600;
            font-size: 0.65rem; /* Lencana lebih kecil di HP */
        ...
        /* Penyesuaian Grid Card untuk Tablet & Desktop */
                ...
                <div class="col-lg-6 order-1 order-lg-2">
                    <div class="card border-0 shadow-sm rounded-4 p-3 p-md-4" style="background-color: #f4fafd;">
                        ...
                        <form action="https://wa.me/6281213663184" method="GET" target="_blank">
                            ...
                            <small class="text-muted mt-2 d-block text-center" style="font-size: 0.75rem;">*Testimoni akan ditampilkan</small>
                        </form>
                    </div>
                </div>
            </div>
        ...
        <div class="container">
            <div class="row g-4">
                ...
                <div class="col-md-4">
                    ...
                    <ul class="list-unstyled text-muted small mt-2 mt-md-3 lh-lg">
                        <li><i class="bi bi-instagram me-2"></i> @vegetablefruit.mdn</li>
                        <li><i class="bi bi-whatsapp me-2"></i> 0812-1366-3184</li>
                    </ul>
                </div>
            </div>
